...
impl Cpu {
    ...
    pub fn execute_unprefixed(&mut self, bus: &mut impl Bus, opcode: u8) -> u32 {
        match opcode {
            ...
            0x01 => { let val = self.fetch_word(bus); self.registers.set_bc(val); 12 }
            0x11 => { let val = self.fetch_word(bus); self.registers.set_de(val); 12 }
            0x21 => { let val = self.fetch_word(bus); self.registers.set_hl(val); 12 }
            0x31 => { let val = self.fetch_word(bus); self.registers.sp = val; 12 }

            // 16-bit Indirect Loads / Stores
            0x02 => { bus.write_byte(self.registers.bc(), self.registers.a); 8 }
            0x12 => { bus.write_byte(self.registers.de(), self.registers.a); 8 }
            0x0A => { self.registers.a = bus.read_byte(self.registers.bc()); 8 }
            0x1A => { self.registers.a = bus.read_byte(self.registers.de()); 8 }
            ...
            0x03 => { self.registers.set_bc(self.registers.bc().wrapping_add(1)); 8 }
            0x13 => { self.registers.set_de(self.registers.de().wrapping_add(1)); 8 }
            0x23 => { self.registers.set_hl(self.registers.hl().wrapping_add(1)); 8 }
            0x33 => { self.registers.sp = self.registers.sp.wrapping_add(1); 8 }
            0x0B => { self.registers.set_bc(self.registers.bc().wrapping_sub(1)); 8 }
            0x1B => { self.registers.set_de(self.registers.de().wrapping_sub(1)); 8 }
            0x2B => { self.registers.set_hl(self.registers.hl().wrapping_sub(1)); 8 }
            0x3B => { self.registers.sp = self.registers.sp.wrapping_sub(1); 8 }

            // 8-bit INC r8 / (HL)
            0x04 => { self.op_inc_r8(bus, R8::B); 4 }
            0x0C => { self.op_inc_r8(bus, R8::C); 4 }
            0x14 => { self.op_inc_r8(bus, R8::D); 4 }
            0x1C => { self.op_inc_r8(bus, R8::E); 4 }
            0x24 => { self.op_inc_r8(bus, R8::H); 4 }
            0x2C => { self.op_inc_r8(bus, R8::L); 4 }
            0x34 => { self.op_inc_r8(bus, R8::HL); 12 }
            0x3C => { self.op_inc_r8(bus, R8::A); 4 }

            // 8-bit DEC r8 / (HL)
            0x05 => { self.op_dec_r8(bus, R8::B); 4 }
            0x0D => { self.op_dec_r8(bus, R8::C); 4 }
            0x15 => { self.op_dec_r8(bus, R8::D); 4 }
            0x1D => { self.op_dec_r8(bus, R8::E); 4 }
            0x25 => { self.op_dec_r8(bus, R8::H); 4 }
            0x2D => { self.op_dec_r8(bus, R8::L); 4 }
            0x35 => { self.op_dec_r8(bus, R8::HL); 12 }
            0x3D => { self.op_dec_r8(bus, R8::A); 4 }

            // 8-bit LD r8, n8 / (HL), n8
            0x06 => { let val = self.fetch_byte(bus); self.registers.b = val; 8 }
            0x0E => { let val = self.fetch_byte(bus); self.registers.c = val; 8 }
            0x16 => { let val = self.fetch_byte(bus); self.registers.d = val; 8 }
            0x1E => { let val = self.fetch_byte(bus); self.registers.e = val; 8 }
            0x26 => { let val = self.fetch_byte(bus); self.registers.h = val; 8 }
            0x2E => { let val = self.fetch_byte(bus); self.registers.l = val; 8 }
            0x36 => { let val = self.fetch_byte(bus); bus.write_byte(self.registers.hl(), val); 12 }
            0x3E => { let val = self.fetch_byte(bus); self.registers.a = val; 8 }
            ...
            // ADD HL, r16
            0x09 => { self.op_add_hl(self.registers.bc()); 8 }
            0x19 => { self.op_add_hl(self.registers.de()); 8 }
            0x29 => { self.op_add_hl(self.registers.hl()); 8 }
            0x39 => { self.op_add_hl(self.registers.sp); 8 }

            // JR e8 & JR cc, e8
            0x18 => { self.op_jr_unconditional(bus); 12 }
            ...
            0x40..=0x7F => {
                if opcode == 0x76 {
                    ...
                    if src_reg == R8::HL || dst_reg == R8::HL { 8 } else { 4 }
                }
            }
            ...
            0xC9 => { self.registers.pc = self.pop_stack_16(bus); 16 }
            0xD9 => {
            ...
            // POP & PUSH r16
            0xC1 => { let val = self.pop_stack_16(bus); self.registers.set_bc(val); 12 }
            0xD1 => { let val = self.pop_stack_16(bus); self.registers.set_de(val); 12 }
            0xE1 => { let val = self.pop_stack_16(bus); self.registers.set_hl(val); 12 }
            0xF1 => { let val = self.pop_stack_16(bus); self.registers.set_af(val); 12 } // Masked to 0xF0

            0xC5 => { self.push_stack_16(bus, self.registers.bc()); 16 }
            0xD5 => { self.push_stack_16(bus, self.registers.de()); 16 }
            0xE5 => { self.push_stack_16(bus, self.registers.hl()); 16 }
            0xF5 => { self.push_stack_16(bus, self.registers.af()); 16 }
            ...
            0xC3 => { self.registers.pc = self.fetch_word(bus); 16 }
            0xE9 => { self.registers.pc = self.registers.hl(); 4 }
            ...
            // RST vectors
            0xC7 => { self.op_rst(bus, 0x0000); 16 }
            0xCF => { self.op_rst(bus, 0x0008); 16 }
            0xD7 => { self.op_rst(bus, 0x0010); 16 }
            0xDF => { self.op_rst(bus, 0x0018); 16 }
            0xE7 => { self.op_rst(bus, 0x0020); 16 }
            0xEF => { self.op_rst(bus, 0x0028); 16 }
            0xF7 => { self.op_rst(bus, 0x0030); 16 }
            0xFF => { self.op_rst(bus, 0x0038); 16 }
            ...
            0xF3 => { self.ime_state = crate::cpu::ImeState::Disabled; 4 }
            0xFB => { self.ime_state = crate::cpu::ImeState::PendingEnable; 4 }
            0x10 => { self.fetch_byte(bus); self.stopped = true; 4 } // STOP 0
    ...
    pub fn execute_cb(&mut self, bus: &mut impl Bus, cb_opcode: u8) -> u32 {
        ...
        match group {
            0 => { // Rotate / Shift / SWAP
                ...
                self.registers.set_f(res.flags);
                if reg == R8::HL { 16 } else { 8 }
            }
            1 => { // BIT b, r8
                let flags = Alu::bit(val, bit, self.registers.f);
                self.registers.set_f(flags);
                if reg == R8::HL { 12 } else { 8 }
            }
            2 => { // RES b, r8
